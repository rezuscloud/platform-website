(() => {
  ...
  function fitRect(rect) {
    const overscan = rect === targets.get("terminal") ? 1.045 : 1
  ...
  function computeProgress(scrollY) {
    ...
    if (raw < 0.18) return { phase: 0, mix: 0, overall: 0 }
    if (raw < 0.46) {
      const local = easeOutExpo((raw - 0.18) / 0.28)
      return { phase: 0, mix: local, overall: raw }
    }
    if (raw < 0.62) return { phase: 1, mix: 0, overall: raw }
    if (raw < 0.9) {
      const local = easeOutExpo((raw - 0.62) / 0.28)
  ...
  function applyScene(scrollY) {
    ...
    if (!terminal || !mac || !linux) return

    const state = computeProgress(scrollY)
    let activeRect = terminal

    if (state.phase === 0 && state.mix > 0) {
      activeRect = blendRect(terminal, mac, state.mix)
    } else if (state.phase === 1 && state.mix === 0) {
      activeRect = mac
    } else if (state.phase === 1 && state.mix > 0) {
      activeRect = blendRect(mac, linux, state.mix)
    } else if (state.phase === 2) {
      activeRect = linux
    }

    const fitted = fitRect(activeRect)
